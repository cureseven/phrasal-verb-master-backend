...
### フェーズ1: 認証基盤（最優先）
- [ ] `auth-01-backend-signup-login`: backend — `POST /api/auth/signup`, `POST /api/auth/login`（bcryptjsでハッシュ化、JWT発行、HTTP-Only Cookieにセット）
- [ ] `auth-02-backend-logout-middleware`: backend — `POST /api/auth/logout`、JWT検証ミドルウェア（`requireAuth`）の実装
